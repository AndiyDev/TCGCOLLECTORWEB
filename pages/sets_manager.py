...
st.set_page_config(page_title="Sets Manager", page_icon="🗃️", layout="wide")
    ...
    if submit:
        if not set_name or not set_id or not symbol_url:
            ...
            for i in range(1, total_cards + 1):
                card_num = str(i)
                # Pokémon.com struktur: /series/[set_id]/[card_num]/
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        # Spara i global_cards (Master-tabellen)
                        with conn.session as s:
                            s.execute(text("""
                ...
                # Uppdatera UI
                progress = i / total_cards
                progress_bar.progress(progress)
                status_text.text(f"Bearbetar kort {i} av {total_cards}...")
                time.sleep(0.2) # Förhindra rate-limit
    ...
    if not sets_df.empty:
        for _, row in sets_df.iterrows():
            with st.expander(f"📦 {row['set_name']} (ID: {row['set_intern_id']})"):
                ...
                # Info
                count_query = f"SELECT COUNT(*) as c FROM global_cards WHERE set_intern_id = '{row['set_intern_id']}'"
                current_cards = conn.query(count_query).iloc[0]['c']
                ...
                if c3.button("🔄 Uppdatera Priser", key=f"btn_prc_{row['set_intern_id']}"):
                    ...
                    # Här ansluter vi Cardmarket-logiken senare
                    time.sleep(1)
                    st.success("Priser synkade mot NM-snitt!")
                
                if c3.button("🗑️ Radera Set", key=f"btn_del_{row['set_intern_id']}", type="secondary"):
                    st.error("Är du säker? Detta raderar även alla kort i biblioteket!")
                    # Logik för radering kan läggas här vid behov

...
import time
import pandas as pd
from database import get_conn, add_item_to_user, create_booster_opening
...
# --- SESSION STATE FÖR OPENING RESULTS ---
                ...
                for _, card in search_results.iterrows():
                    with st.container(border=True):
                        ...
                        with c_info:
                            ...
                            is_wishlist = st.toggle("Lägg till önskelista istället", value=False, key=f"wish_{card['api_id']}")
                            is_bought = not is_wishlist
...
# --- MODUS 2: BOOSTER OPENING ---
    ...
    # BUG FIX #9a: Guard against empty sets list to prevent crash on .values[0]
    if available_sets.empty:
        st.warning("Inga sets hittades i biblioteket. Importera ett set i 'Sets Manager' först!")
        st.stop()

    # BUG FIX #9b: Moved skip_anim checkbox OUTSIDE the opening button block
    # so it is rendered before the loop and doesn't cause widget re-render errors
    skip_anim = st.checkbox("Skippa animation", value=False)
    ...
    with st.expander("1. Konfigurera din Booster", expanded=True):
        ...
        pack_price = col_p.number_input("Vad betalade du för paketet? (SEK)", min_value=0.0, value=65.0)
    ...
    if st.button("✨ ÖPPNA BOOSTER ✨", type="primary", use_container_width=True):
        # Validate all 10 card numbers are filled
        empty_slots = [i + 1 for i, n in enumerate(card_numbers) if not n.strip()]
        if empty_slots:
            st.error(f"Fyll i kortnummer för kort: {empty_slots}")
        else:
            valid_cards = []
            error = False
    ...
    if st.session_state.opening_results:
        ...
        c1.metric("Kostnad", f"{res['price']:.2f} SEK")
        c2.metric("Värde (NM)", f"{res['value']:.2f} SEK")
        profit = res['value'] - res['price']
        c3.metric("ROI", f"{profit:.2f} SEK", delta=f"{profit:.2f}", delta_color="normal")
